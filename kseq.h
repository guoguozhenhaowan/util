...
#define __KSEQ_BASIC(SCOPE, type_t)										\
	...
	SCOPE krec_t *krec_init()             \
        {                                      \
            krec_t *r = (krec_t*)calloc(1, sizeof(krec_t)); \
            return r; \
        }   \
	...
	{																	\
        ...
        SCOPE void krec_destroy(krec_t *kr)                    \
        {      \
                if(!kr) return; \
                free(kr->name.s); free(kr->comment.s); free(kr->seq.s); free(kr->qual.s); \
                free(kr);\
        }\
...
#define __KSEQ_READ(SCOPE) \
	SCOPE int kseq_read(kseq_t *seq, krec_t* rec) \
	{ \
		int c,r; \
		kstream_t *ks = seq->f; \
		...
		if ((r=ks_getuntil(ks, 0, &rec->name, &c)) < 0) return r;  /* normal exit: EOF or error */ \
		if (c != '\n') ks_getuntil(ks, KS_SEP_LINE, &rec->comment, 0); /* read FASTA/Q comment */ \
		...
		while ((c = ks_getc(ks)) >= 0 && c != '>' && c != '+' && c != '@') { \
			...
			ks_getuntil2(ks, KS_SEP_LINE, &rec->seq, 0, 1); /* read the rest of the line */ \
		} \
		...
		while ((c = ks_getuntil2(ks, KS_SEP_LINE, &rec->qual, 0, 1) >= 0 && rec->qual.l < rec->seq.l)); \
...
#define KSEQ_DECLARE(type_t) \
	...
	int kseq_read(kseq_t *seq, krec_t* rec);
